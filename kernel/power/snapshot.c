...
static unsigned int key_bloblen;
...
int snapshot_image_verify(void)
{
	...
	ret = init_snapshot_key_by_blob(encrypted_key_blob, key_bloblen);
 	if (ret)
		goto error_prep;
...
static int init_header(struct swsusp_info *info)
{
	...
	init_signature(info);
	info->key_bloblen = get_encrypted_snapshot_key_blob(info->encrypted_key_blob);
